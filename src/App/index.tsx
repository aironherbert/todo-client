...
export default function App() {
  ...
  useEffect(() => {
    if (data) {
      setUser({
        ...
        id: data?.profile?.id ?? undefined,
      });
    }
  }, [data, data?.profile]);

  const [createTodo] = useMutation(CREATE_TODO, {
    refetchQueries: [GET_USER, "profile"],
  });
  ...
  }>({ id: null, title: "", description: "", color: "#000000", done: false });
    ...
    <div className="container">
      ...
      >
        ...
        <Button
          ...
          color={`${state === "create" ? "secondary" : "primary"}`}
          onClick={() => setState(state === "create" ? "" : "create")}
        >
          {state === "create" ? "Fechar" : "Criar nova atividade"}
        </Button>
        {state === "create" && (
          <Card
            className="create"
      ...
      {loading
        ? "Carregando..."
        : data?.profile?.todos?.map((todo: Todo) => {
      ...
      >
        ...
        >
          ...
          <Button
            ...
            onClick={() => {
              updateTodo({
                variables: { id: selected.id, color: selected.color },
              });
              setState("");
            }}
          >
